#!/usr/bin/env python3
"""
Download Cancer/Malignant Breast DICOM Series from TCIA

Similar to download_tcia_normals.py but filters for cancer/malignant cases.
"""
...
def classify_series_meta(series_meta: Dict) -> Tuple[bool, str]:
    """Heuristically classify a series as cancer/malignant.
    
    Returns (is_cancer, reason)
    """
    fields = []
    for key in [
        "SeriesDescription",
    ...
    if text_has_any(blob, CANCER_KEYWORDS):
        return True, "cancer_keyword"

    # Unknown/neutral metadata: treat as unknown
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Download TCIA cancer/malignant breast DICOM series")
    parser.add_argument("--output-dir", default="dicom_download_cancer", help="Base output directory (default: dicom_download_cancer)")
    parser.add_argument("--collection", default=None, help="TCIA collection name (e.g., TCGA-BRCA, CBIS-DDSM, INbreast)")
    parser.add_argument("--modality", default="MG", choices=["MR", "MG", "CT", "PT", "CR"], help="Modality filter (default: MG)")
    parser.add_argument("--max-series", type=int, default=50, help="Max number of series to download (default: 50)")
    parser.add_argument("--strict-only", action="store_true", help="Include only series explicitly labeled as cancer/malignant")
    parser.add_argument("--dry-run", action="store_true", help="List series that would be downloaded without downloading")
    parser.add_argument("--cbis-csv", nargs="*", help="One or more CBIS-DDSM metadata CSV files with pathology labels (used to filter malignant)")
    ...
    # Optional: For CBIS-DDSM, use provided CSV(s) to keep only malignant PatientIDs
    malignant_patient_ids: set = set()
    allowed_series_uids: set = set()
    allowed_study_uids: set = set()
    malignant_core_ids: set = set()
    if args.collection and str(args.collection).upper() == "CBIS-DDSM" and (args.cbis_csv or args.cbis_csv_url):
        ...
        for df in dfs:
            ...
            for _, row in df.iterrows():
                ...
                if not pid:
                    continue
                if any(k in pathology for k in ["malignant", "cancer", "invasive", "ductal", "lobular", "carcinoma"]):
                    malignant_patient_ids.add(pid)
                    core = extract_core_id(pid)
                    if core:
                        malignant_core_ids.add(core)
        ...
        print(f"   CBIS-DDSM CSV filter loaded: {len(malignant_patient_ids)} malignant PatientIDs, "
              f"{len(malignant_core_ids)} core IDs, {len(allowed_series_uids)} series UIDs, {len(allowed_study_uids)} study UIDs")

    # Heuristic filtering for cancer, with optional CBIS-DDSM patient filter
    ...
    norm_malignant_ids = {norm_id(p) for p in malignant_patient_ids}
    norm_series_uids = {norm_id(u) for u in allowed_series_uids}
    norm_study_uids = {norm_id(u) for u in allowed_study_uids}
    norm_malignant_cores = {norm_id(x) for x in malignant_core_ids}

    for s in series:
        # If CBIS malignant CSV constraints are present, try multiple matching strategies
        if malignant_patient_ids or allowed_series_uids or allowed_study_uids:
            ...
            if s_series_uid and s_series_uid in norm_series_uids:
                candidates.append({**s, "_is_cancer": True, "_reason": "cbis_csv_series_uid"})
                continue
            if s_study_uid and s_study_uid in norm_study_uids:
                candidates.append({**s, "_is_cancer": True, "_reason": "cbis_csv_study_uid"})
                continue
            # accept if any pid candidate matches
            if any(pid in norm_malignant_ids for pid in pid_candidates):
                candidates.append({**s, "_is_cancer": True, "_reason": "cbis_csv_patient"})
                continue
            # accept if any derived core id matches
            if any(core in norm_malignant_cores for core in core_candidates):
                candidates.append({**s, "_is_cancer": True, "_reason": "cbis_csv_patient_core"})
                continue
            # no match => skip
            continue

        is_cancer, reason = classify_series_meta(s)
        if args.strict_only:
            if is_cancer and reason == "cancer_keyword":
                candidates.append({**s, "_is_cancer": True, "_reason": reason})
        else:
            # Lenient mode: include series that contain cancer terms
            if reason == "cancer_keyword":
                candidates.append({**s, "_is_cancer": True, "_reason": reason})

    print(f"   After heuristic filtering => {len(candidates)} candidate 'cancer/malignant' series")
    if args.dry_run:
            ...
            manifest_rows.append(
                {
                    ...
                    "modality": args.modality,
                    "is_cancer": str(s.get("_is_cancer", False)),
    ...
    # Write manifest
    manifest_path = os.path.join(args.output_dir, "manifest_cancer.csv")
